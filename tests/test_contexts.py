...
class ContextStoreTests(unittest.TestCase):
    ...
    def test_upsert_use_and_tags(self) -> None:
        self.store.upsert_context(
            ...
            tags=["gh:example/demo"],
            activate=True,
        )
        active = self.store.get_active_context_name()
        self.assertEqual(active, "demo")
        ctx = self.store.get_context("demo")
        assert ctx is not None
        self.assertEqual(ctx.project_key, "local-demo")
        self.assertIn("gh:example/demo", ctx.tags or [])
        ...
        self.assertFalse(self.store.get_context("demo").active)  # type: ignore[union-attr]
